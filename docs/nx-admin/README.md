...
- Supabase-authenticated access control (sign-in wall)
...
NX° Admin requires Supabase to be configured via environment variables:
...
NEXT_PUBLIC_SUPABASE_VAPID_KEY=   # Required for push notifications
```
...
```
<NXAdmin config={…}>
  ├── useSupabaseAuthListener()      ← subscribes to Supabase Auth state
...
1. `useSupabaseAuthListener()` subscribes to Supabase Auth state changes.
